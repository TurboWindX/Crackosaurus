...
export const AddDialog = ({ typeSingular, open, setOpen, onSubmit, children }: AddDialogProps) => {
  return (
    <DrawerDialog
...
export const RemoveDialog = ({ typePlural, open, setOpen, count, onSubmit }: RemoveDialogProps) => {
  return (
    <DrawerDialog
    ...
    >
      <div className="grid gap-4">
        <span>Do you want to remove {count} {typePlural.toLowerCase()}?</span>
...
export function DataTable<T>({ typeSingular, typePlural, onAdd, onRemove, row, valueKey, addDialog, values, head, searchFilter, noAdd, noRemove }: DataTableProps<T>) {
  ...
  const searchValues = values.filter((value) => searchFilter?.(value, search) ?? true); 
  const selectedValues = searchValues.filter((value) => selects[valueKey(value)]);
  ...
  return (
    <div className="grid gap-4">
          ...
          <div className="grid gap-4 w-max grid-flow-col">
            ...
            {hasRemove &&
              <RemoveDialog
                ...
                onSubmit={async () => {
                  if (await onRemove?.(selectedValues)) setRemoveDialogOpen(false);
                }}
              />
            }
          </div>
        ...
        {searchValues.length === 0 
          ? (
            <Card className="p-4 grid justify-center">
              No {plural} Found
            </Card>
          )
          : (
                      ...
                      <TableHead className="w-[50px]">
                        <Checkbox
                          ...
                          onCheckedChange={(state) => {
                            ...
                            setSelects({
                              ...selects,
                              ...Object.fromEntries(searchValues.map((value) => [valueKey(value), val] as const))
                            });
                          }}
                        />
                      </TableHead>
                    }
                    {head.map((label) => <TableHead>{label}</TableHead>)}
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {
                        ...
                        {hasSelect &&
                          <TableCell>
                            <Checkbox
                              ...
                              onCheckedChange={(state) => setSelects({
                                ...selects,
