...
export async function createJob(
    ...
    if (hashes.length === 0) throw new APIError("Cannot create a job without any valid hashes");

    const hashType = hashes[0]?.hashType;
    if (hashes.some((hash) => hash.hashType !== hashType)) throw new APIError("Cannot create jobs with different types of hashes");

    const instanceTag = await instanceAPI.create(hashType as any, hashes.map((hash) => hash.hash), instanceType);
...
export async function createProjectJobs(
    ...
    const jobIds = await Promise.all(Object.entries(hashByType).map(([_hashType, hashIds]) => createJob(prisma, instanceAPIs, provider, hashIds, currentUserID, bypassCheck, instanceType)));
...
export async function startJob(
    ...
    const instanceAPI = instanceAPIs[job.instance.provider as keyof InstanceAPIProviders];
    if (!instanceAPI) throw new Error("Instance API not found"); 

    if (job.status === "STARTED") throw new APIError("Job already started");
    if (job.instance.status === "STARTED") throw new APIError("Instance already started");

    if (!await instanceAPI.start(job.instance.tag)) throw new APIError("Instance could not start");
...
export async function stopJob(
    ...
    if (job.instance.status !== "STARTED") throw new APIError("Instance already not running");

    if (!await instanceAPI.stop(job.instance.tag)) throw new APIError("Instance could not start");
