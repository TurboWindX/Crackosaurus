...
export class AWSCluster extends FileSystemCluster<AWSClusterConfig> {
  ...
  protected async run(instanceID: string): Promise<void> {
    ...
    console.log(`[AWS Cluster] Starting Step Functions execution with input:`, JSON.stringify(stepFunctionInput));
  ...
  public async deleteInstance(instanceID: string): Promise<boolean> {
    console.log(`[AWS Cluster] deleteInstance() called for instanceID: ${instanceID}`);
    ...
    try {
      ...
      if (metadata.ec2InstanceId) {
        console.log(`[AWS Cluster] Terminating EC2 instance: ${metadata.ec2InstanceId}`);
        ...
        try {
          ...
          console.log(`[AWS Cluster] EC2 instance ${metadata.ec2InstanceId} termination initiated`);
        } catch (e: any) {
          // Ignore "InvalidInstanceID.NotFound" errors (instance already terminated)
          if (e.code === 'InvalidInstanceID.NotFound') {
            console.log(`[AWS Cluster] EC2 instance ${metadata.ec2InstanceId} already terminated`);
          } else {
        ...
        console.log(`[AWS Cluster] No EC2 instance ID found in metadata, instance may not have been started yet or already deleted`);
      }
    } catch (e) {
  ...
  public async createJob(
    ...
    if (!jobID) {
      console.log(`[AWS Cluster] Failed to create job for instance ${instanceID}`);
      return null;
    }
    ...
    if (this.config.jobQueueUrl) {
      try {
        ...
        console.log(`[AWS Cluster] Sent SQS notification for job ${jobID} on instance ${instanceID}`);
      } catch (e) {
        console.error(`[AWS Cluster] Failed to send SQS message:`, e);
      }
    } else {
      console.log(`[AWS Cluster] No SQS queue URL configured, skipping notification`);
    }
