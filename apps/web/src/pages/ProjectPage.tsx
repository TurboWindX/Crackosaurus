...
import { AddHashRequest, GetProjectResponse, HASH_TYPES } from "../../../../packages/api/src/types.ts";
import { addHashToProject, deleteProject, getProject } from "../../../../packages/api/src/web.ts";
...
export const ProjectPage = () => {
  ...
  const [project, setProject] = useState<GetProjectResponse["response"] | null>(null);
  const [removeOpen, setRemoveOpen] = useState(false);

  const [addHash, setAddHash] = useState<AddHashRequest["Body"]>({ hash: "", hashType: "" });
    ...
    return <div>
      ...
      <div className="p-4 grid gap-8">
            ...
            <div className="w-max">
              ...
              >
                <form className="grid gap-4" onSubmit={async (e) => {
                  ...
                  const res = await deleteProject(parseInt(projectID ?? "-1"));
        ...
        <DataTable
          ...
          addDialog={<>
            ...
            <Select
              ...
              onValueChange={(value) => setAddHash({
                ...addHash,
                hashType: value
              })}
            >
              ...
              <SelectContent>
                {HASH_TYPES.map((type) => <SelectItem value={type}>{type}</SelectItem>)}
              </SelectContent>
            </Select>
          </>}
        />
        ...
        <DataTable
          ...
          searchFilter={({ username }, search) => username.toLowerCase().includes(search)}
        />
      </div>
    </div>;
  } else {
