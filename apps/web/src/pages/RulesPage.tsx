...
export const RulesPage = () => {
  ...
  const queryKeys = useMemo(
    () => [getQueryKey(trpc.rules.getMany, undefined, "any"), getQueryKey(trpc.rules.getList, undefined, "any")],
    []
  );
  ...
  const {
    ...
    error,
    isLoadingError,
  } = trpc.rules.getMany.useQuery(undefined, {
    retry(count, error) {
      if (
  ...
  const { mutateAsync: uploadRule } = useMutation<void, Error, { file: File; onProgress?: (percent: number) => void; abortSignal?: AbortSignal }>(
    {
      mutationFn: async ({ file, onProgress, abortSignal }) => {
        await upload.rules(file, onProgress, abortSignal);
      },
      onSuccess() {
        setProgress(100);
        setTimeout(() => {
  ...
  const { mutateAsync: deleteRules } = trpc.rules.deleteMany.useMutation({
    onSuccess() {
      queryKeys.forEach((key) => queryClient.invalidateQueries(key));
    },
    onError: handleError,
